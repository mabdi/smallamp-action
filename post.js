...
//         fs.writeFileSync(`${dir}/amp-${job_id}.txt`, myOutput, function(error){ core.error(error) });
//         let myOutput = '';
//         await exec.exec('python3', ['runner.py', '-r', 'sum', '-d', base, '-p', process.env.reponame], options);
//         fs.writeFileSync(`${dir}/sum-${job_id}.txt`, myOutput, function(error){ core.error(error) }) 
//     } catch (error) {
